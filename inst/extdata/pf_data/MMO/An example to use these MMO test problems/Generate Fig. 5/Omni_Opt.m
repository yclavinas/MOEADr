function [ps,pf,Dis_proportion]=Omni_Opt(fname,n_obj,n,xl,xu,pop,Maxgen)
...
%    disp(['Omni_opt  gen=  ' num2str(i) ]);
    ...
    mu = 20;
    mum = 20;
    offspring_chromosome = genetic_operator(parent_chromosome, n_obj, n, mu, mum, xl, xu,fname,feval_count);
    ...
    particle = replace_chromosome(intermediate_chromosome, n_obj, n, pop);
     Dis_proportion(i,:)=calculate_region_proportion (particle(:,1:n));
...
%    objective = [];
    ...
    for i = 1 : M+V
        ...
        if length(index_of_objectives)==1 %if there is only one point in the current front
        ...
        elseif i>V
            ...
            % the boundary point in decision space: twice the distance
            % btween the boundary and its nearest neighbor
...
function f  = genetic_operator(parent_chromosome, M, V, mu, mum, l_limit, u_limit,fname,feval_count)
